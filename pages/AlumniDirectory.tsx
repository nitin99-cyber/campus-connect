...
import { MatchingService, StudentPreferences, MatchResult } from '../services/matchingService';
import { Search, Filter, GraduationCap, Building2, Briefcase, Mail, MessageSquare, Linkedin, X, CheckCircle2, AlertTriangle, Loader2, Calendar, MapPin, Quote, Layers, Sparkles, BrainCircuit } from 'lucide-react';
...
const MOCK_DEMO_STUDENT: StudentPreferences = {
  branch: "CSE",
  interestedDomain: "Software",
  careerGoal: "Mentorship",
  preferredExperience: "5-10",
  passoutPreference: "Senior"
};
...
export const AlumniDirectory: React.FC = () => {
  ...
  const handleSmartMatch = () => {
    ...
    const matches = MatchingService.findMatches(MOCK_DEMO_STUDENT, alumni);
    setMatchResults(matches);
  };

  const displayedAlumni = useMemo(() => {
    if (isSmartMatching) {
        return matchResults.map(m => m.alumni);
    }
  ...
  }, [searchQuery, selectedBranch, selectedDomain, minExp, alumni, isSmartMatching, matchResults]);
  ...
  return (
    <div className="space-y-12 pb-20">
      ...
      <div className="max-w-6xl mx-auto sticky top-[80px] z-30 px-4 md:px-0">
          <div className="bg-panel/90 backdrop-blur-xl border border-white/10 rounded-2xl p-2 flex gap-2 shadow-elev-2 flex-wrap">
             ...
             <button
               ...
               className={`px-6 py-2 rounded-xl font-ui font-bold transition-all flex items-center gap-2 shadow-lg ${
                   isSmartMatching 
                   ? 'bg-gradient-to-r from-pink-500 to-rose-500 text-white shadow-rose-500/20' 
                   : 'bg-gradient-to-r from-violet-600 to-indigo-600 text-white hover:shadow-violet-500/30'
               }`}
             >
                {isSmartMatching ? <X className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
                {isSmartMatching ? 'Clear Match' : 'Smart Match'}
             </button>
          ...
          {/* Smart Match Context Banner */}
          {isSmartMatching && (
              <div className="mt-4 bg-gradient-to-r from-violet-900/50 to-indigo-900/50 border border-violet-500/30 rounded-2xl p-4 animate-fadeIn flex items-start gap-4">
                  <div className="p-3 bg-violet-500/20 rounded-full text-violet-300">
                      <BrainCircuit className="w-6 h-6" />
                  </div>
                  <div>
                      <h3 className="text-white font-bold text-sm mb-1">AI Matching Active (Demo Mode)</h3>
                      <p className="text-violet-200 text-xs">
                          Showing top matches for: <span className="font-bold text-white">CSE Student</span> interested in <span className="font-bold text-white">Software</span> seeking <span className="font-bold text-white">Mentorship</span>.
                      </p>
                  </div>
              </div>
          )}
      ...
      {loading ? (
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-4 md:px-0">
           {displayedAlumni.map((alum) => {
             // Find match data if smart matching is on
             const matchData = isSmartMatching ? matchResults.find(m => m.alumni.id === alum.id) : null;

             return (
             <RevealOnScroll key={alum.id} threshold={0.05}>
               <div 
                 onClick={() => setSelectedAlumni(alum)}
                 className={`bg-panel border rounded-[24px] p-6 transition-all hover:-translate-y-1 group relative overflow-hidden h-full flex flex-col cursor-pointer ${matchData ? 'border-violet-500/40 shadow-[0_0_20px_rgba(139,92,246,0.15)]' : 'border-white/5 hover:border-white/10'}`}
               >
                  {/* Banner Decoration */}
                  ...
                  <div className="relative z-10 flex flex-col items-center text-center mt-4">
                     ...
                     <div className="flex flex-col items-center justify-center">
                        ...
                        <VerificationBadge level={alum.verificationLevel} className="scale-90" />
                     </div>
                     ...
                     {matchData && (
                         <div className="mt-3 flex flex-col items-center gap-2 w-full">
                             <div className="px-3 py-1 bg-violet-500 text-white text-xs font-bold rounded-full shadow-lg flex items-center gap-1">
                                 <Sparkles className="w-3 h-3" /> {matchData.score}% Match
                             </div>
                             <div className="flex flex-wrap justify-center gap-1">
                                 {matchData.reasons.slice(0,2).map((r, idx) => (
                                     <span key={idx} className="text-[9px] bg-violet-500/10 text-violet-300 px-2 py-0.5 rounded border border-violet-500/20">{r}</span>
                                 ))}
                             </div>
                         </div>
                     )}
                ...
                <div className="flex-1 overflow-y-auto px-8 pb-8 -mt-12 custom-scrollbar relative z-10">
                    <div className="flex flex-col md:flex-row gap-6 items-start md:items-end mb-8">
                        ...
                        <div className="flex-1 min-w-0">
                            <h2 className="text-3xl font-heading font-bold text-white mb-1 flex items-center gap-2">
                                {alumni.name}
                                <VerificationBadge level={alumni.verificationLevel} className="scale-90" />
                            </h2>
