...
export type VerificationLevel = 'self' | 'peer' | 'faculty';
...
export interface Student {
  ...
  quizScore: number;
  isMock?: boolean;
  verificationLevel?: VerificationLevel; // Trust Signal
}
...
export interface Project {
  ...
  techStack: string[];
  link?: string;
  verificationStatus?: VerificationLevel; // Trust Signal for specific projects
}
...
export interface CollaborationPost {
  ...
  postedAt: string;
  openRoles: number;
  // Extended fields for Event details
  prizePool?: string;
  guidelines?: string;
  instructions?: string;
}
