...
export const StudentProfileModal: React.FC<{
    ...
    const handleShare = () => {
        try {
            // Check if we are in a safe context to read location
            const origin = window.location.origin && window.location.origin !== 'null' ? window.location.origin : 'https://campusconnect.com';
            const url = `${origin}/#/student/${student.id}`;
            navigator.clipboard.writeText(url).then(() => {
                setCopied(true);
                setTimeout(() => setCopied(false), 2000);
            }).catch(err => console.error("Clipboard write failed", err));
        } catch (e) {
            console.warn("Share functionality disabled in this environment.");
        }
    };
